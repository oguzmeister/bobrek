...
# 2. MODEL YÜKLEME VE DİNAMİK YOL ÇÖZÜMÜ (Madde 19)
@st.cache_resource
def load_trained_model():
    # Çalışma dizinini garantiye alıyoruz
    current_dir = os.path.dirname(os.path.abspath(__file__))
    model_name = 'kidney_disease_mobilenet_model2.h5'
    model_path = os.path.join(current_dir, model_name)
    
    if not os.path.exists(model_path):
        # Eğer yolda bulamazsa ana dizinde tekrar dene
        if os.path.exists(model_name):
            model_path = model_name
        else:
            st.error(f"❌ Model dosyası bulunamadı! Lütfen GitHub'da '{model_name}' dosyasının adını kontrol edin.")
            return None

    # Keras 3 'batch_shape' yaması
    try:
...
# --- CANLI TANI MERKEZİ (HİZALAMA VE BUTON ÇÖZÜLDÜ) ---
    ...
    if up_file:
        ...
        with col_analysis:
            ...
            if model is not None:
                ...
                st.error("Model yüklenemediği için analiz yapılamıyor. Lütfen model dosyasının varlığını kontrol edin.")
    st.markdown('</div>', unsafe_allow_html=True)
