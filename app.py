...
# 2. MODEL YÜKLEME VE H5 YAMASI (Madde 19: Teknik Çalışırlık)
@st.cache_resource
def load_trained_model():
    ...
    # Keras 3 'batch_shape' hatası için H5 müdahalesi
    try:
...
st.title("🛡️ Renal AI: Böbrek Patolojisi Analiz Paneli")
...
if st.session_state.page == "vizyon":
    ...
    with c1:
        ...
        Renal AI, derin öğrenme temelli bir sistem geliştirerek radyolojik görüntülemede klinik karar destek mekanizması oluşturmayı hedefler.
        Sistem, kist, taş ve tümör gibi yapıları otomatik olarak saptayarak hekimlerin teşhis hızını artırır.
        """)
    with c2:
...
# ANALİTİK RAPORLAR (Accuracy/Loss Dahil)
    ...
    with g1:
        if os.path.exists('kidney_confusion_matrix.png'): 
            st.image('kidney_confusion_matrix.png', caption="Karmaşıklık Matrisi", use_container_width=True)
    with g2:
        if os.path.exists('kidney_roc_curve.png'): 
            st.image('kidney_roc_curve.png', caption="ROC Analizi", use_container_width=True)

# CANLI TANI MERKEZİ (KAYMA SORUNU ÇÖZÜLDÜ)
    ...
    if up_file:
        # Sütunları yan yana ve tepeden hizalı yapıyoruz
        col_img, col_analysis = st.columns([1, 1], gap="medium")
        
        img = Image.open(up_file).convert('RGB')
        
        with col_img:
            st.markdown("### 🖼️ Giriş BT Kesiti")
            # Resim genişliğini sabitleyerek sağ tarafın kaymasını engelliyoruz
            st.image(img, caption="Yüklenen Ham Görüntü", width=400)
            ...
            if model is not None:
                ...
                if st.button("🚀 ANALİZİ BAŞLAT"):
                    with st.spinner('Yapay Zeka Taraması Yapılıyor...'):
                        ...
                        st.markdown(f"""
                            <div class="prediction-box" style="border-left: 10px solid {res_color}; padding: 20px; background-color: #1f2937; border-radius: 12px; margin-top:0;">
                        ...
                        # Olasılık Grafiği (Yükseklik resim boyuyla uyumlu hale getirildi)
                        st.write("")
